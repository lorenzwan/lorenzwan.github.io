...
def getBoundingBoxCenter(objects: pm.PyNode) -> tuple[float]:
    """Get bounding box center of objects"""
...
def getCenterPositionBetweenTwo(objectA: pm.PyNode, objectB: pm.PyNode) -> om.MVector: 
    start = om.MVector(pm.xform(objectA, q=True, translation=True, worldSpace=True))
    end = om.MVector(pm.xform(objectB, q=True, translation=True, worldSpace=True))
    return (start + end) / 2
```
